...
const V = (() => {
  ...
  /** Timed captions: [[start, end, text|string[]|{lines: string[]}], ...]. */
  function drawSubs(t, subs, showText = true, captionStyle = {}) {
    if (!showText || !subs) return;
    const s = subs.find(s => t >= s[0] && t < s[1]);
    if (!s) return;
    const fadeIn = Math.max(0.001, Math.min(captionStyle.fadeIn ?? 0.12, (s[1] - s[0]) / 4));
    const fadeOut = Math.max(0.001, Math.min(captionStyle.fadeOut ?? 0.12, (s[1] - s[0]) / 4));
    const k = Math.min(seg(t, s[0], s[0] + fadeIn), 1 - seg(t, s[1] - fadeOut, s[1]));
    if (k <= 0) return;
    ...
    const lines = (typeof raw === 'string' ? [raw]
      ...
      : raw && typeof raw === 'object' ? [raw.en, raw.zh].filter(Boolean)
      : []).map(v => String(v).trim()).filter(Boolean);
    if (!lines.length) return;

    const lightTheme = T.id === 'academic_paper';
    const maxW = captionStyle.maxWidth || subConf.captionMaxWidth || 1700;
    const colors = captionStyle.colors || subConf.captionColors || (lightTheme ? ['#0f172a', '#334155'] : ['#f9fbff', '#d9eaff']);
    const outline = captionStyle.outline || subConf.captionOutline || (lightTheme ? 'rgba(255,255,255,0.9)' : 'rgba(3,8,18,0.82)');
    const shadow = captionStyle.shadow || subConf.captionShadow || (lightTheme ? 'rgba(255,255,255,0.65)' : 'rgba(0,0,0,0.72)');
    const lineGap = captionStyle.lineGap || 65;
    const bottomY = captionStyle.bottomY || 980;
    const centerX = captionStyle.centerX || W / 2;
    ...
    lines.forEach((line, i) => {
      const isCJK = /[\u3040-\u30ff\u3400-\u9fff\uac00-\ud7af]/.test(line);
      const family = captionStyle.fonts?.[i] || (isCJK ? CN : '"Avenir Next", "Segoe UI", sans-serif');
      const weight = captionStyle.weights?.[i] || (isCJK ? 600 : 700);
      let px = i === 0 ? (captionStyle.primarySize || 49) : (captionStyle.secondarySize || 44);
      ctx.font = `${weight} ${px}px ${family}`;
      const measured = ctx.measureText(line).width;
      if (measured > maxW) px = Math.max(30, Math.floor(px * maxW / measured));
      ctx.font = `${weight} ${px}px ${family}`;
      const y = bottomY - (lines.length - i - 1) * lineGap;
      ctx.lineWidth = 5;
      ctx.strokeStyle = outline;
      ctx.shadowColor = shadow;
      ctx.shadowBlur = 16;
      ctx.shadowOffsetY = 3;
      ctx.strokeText(line, centerX, y, maxW);
      ctx.shadowBlur = 0;
      ctx.shadowOffsetY = 0;
      ctx.fillStyle = colors[Math.min(i, colors.length - 1)];
      ctx.fillText(line, centerX, y, maxW);
    });
    ctx.restore();
  }
  ...
  function createApp(cfg) {
    ...
    if (cfg.style?.theme) setTheme(cfg.style.theme);
    else if (cfg.theme) setTheme(cfg.theme);
    ...
    function render(t) {
      ...
      cfg.beats.forEach(([s, e], i) => {
        const enter = cfg.style?.motion?.enterSeconds ?? 0.45;
        const exit = cfg.style?.motion?.exitSeconds ?? 0.45;
        const a = seg(t, s, s + enter) * (1 - seg(t, e - exit, e));
        if (a <= 0) return;
        ctx.save(); ctx.globalAlpha = a;
        cfg.drawBeat(i, t - s, t);
        ctx.restore();
      });
      hud(t, cfg.label, cfg.style?.chrome?.hud);
      drawSubs(t, cfg.subs, showText && cfg.showText !== false, cfg.captions || cfg.style?.captions);
      scanlines();
